...
interface SupplierBaseInfoProps {
  ds: DataSet;
  editable: boolean;
}

export default function Index({ ds, editable }: SupplierBaseInfoProps) {
  return (
    <Form dataSet={ds} columns={4} labelLayout={LabelLayout.vertical}>
      {editable ? (
        <>
          <Lov name="supplierTypeId" tableProps={{ queryFieldsLimit: 5 }} />
          <TextField name="supplierName" />
          <TextField name="creditCode" />
          <div></div>
          <TextField name="shortName" />
          <Lov name="level" tableProps={{ queryFieldsLimit: 5 }} />
          <Lov name="purchaserId" tableProps={{ queryFieldsLimit: 5 }} />
